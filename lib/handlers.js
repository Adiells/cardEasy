...
exports.api.cadastro = async(req, res) => {
    ...
    const image = 'default.png' //aparentemente a p#### do sql nao tem como alterar uma tabela ja existente colocando o valor default em uma coluna, entao la vamos nós fazer isso igual os íncas e estou com preguiça de recriar a tabela e copiar os dados
        if(row){
...
exports.cardapio = (req, res) => {
    ...
    const stmt = db.prepare(`
        SELECT pratos.restaurante_id, pratos.titulo, pratos.preco, pratos.descricao, pratos.imagem, users.username, users.nome, users.description, users.image_url
        FROM pratos
