...
exports.cardapio = (req, res) => {
    const nameIdRestaurante = req.params.name
    const [nome, id] = nameIdRestaurante.split('_')
    const stmt = db.prepare(`
        SELECT pratos.restaurante_id, pratos.titulo, pratos.preco, pratos.descricao, pratos.imagem, users.nome AS nome_restaurante
        FROM pratos
        JOIN users ON pratos.restaurante_id = users.id
        WHERE users.id = ? AND LOWER(REPLACE(users.nome, ' ', '')) = LOWER(?)
    `)

    let result = stmt.all(id, nome)
    if(result.length === 0){
